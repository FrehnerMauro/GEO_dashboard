...
(window as any).showDashboard = function(event?: Event) {
  if (event) event.preventDefault();
  // Will be replaced by app.ts
};

(window as any).showAIAnalysis = function(event?: Event) {
  if (event) event.preventDefault();
  // Will be replaced by app.ts
};

(window as any).showAIReadability = function(event?: Event) {
  if (event) event.preventDefault();
  // Will be replaced by app.ts
};

(window as any).showAnalyses = function(event?: Event) {
  if (event) event.preventDefault();
  // Will be replaced by app.ts
};
