...
export class ReadabilityWorkflow {
    ...
    async startAnalysis(url) {
        ...
        if (gptAnalysisSection)
            gptAnalysisSection.style.display = "none";
        // Initialize progress UI
        if (stepNumber)
            stepNumber.textContent = "1";
        if (stepTitle)
            stepTitle.textContent = "Preparing Analysis";
        if (stepDescription)
            stepDescription.textContent = "Initializing...";
        ...
        `;
            ...
            if (stepNumber)
                stepNumber.textContent = "1";
            if (stepTitle)
                stepTitle.textContent = "Searching for robots.txt...";
            if (stepDescription)
                stepDescription.textContent = "Checking robots.txt file";
            if (progressPercentage)
            ...
            const stepInterval = setInterval(() => {
                if (currentStepIndex < progressSteps.length) {
                    ...
                    if (stepNumber)
                        stepNumber.textContent = step.step.toString();
                    if (stepTitle)
            ...
            const successful = data.protocol?.pages?.filter((p) => p.success).length || 0;
            const failed = data.protocol?.pages?.filter((p) => !p.success).length || 0;
            ...
            if (stepTitle)
                stepTitle.textContent = "Analysis Completed";
            if (stepDescription)
                stepDescription.textContent = "All steps finished successfully";
            if (progressPercentage)
            ...
            if (data.protocol?.analysis) {
                if (gptAnalysisSection)
                    gptAnalysisSection.style.display = "block";
            ...
            if (pagesSummaryDisplay && data.protocol?.pages) {
                ...
                data.protocol.pages.forEach((page) => {
        ...
        `;
            ...
            if (stepTitle)
                stepTitle.textContent = "Error Occurred";
            if (stepDescription)
                stepDescription.textContent = error instanceof Error ? error.message : "Unknown error";
            if (progressPercentage)
                progressPercentage.textContent = "0%";
            if (progressFill)
                progressFill.style.width = "0%";
...
//# sourceMappingURL=readability-workflow.js.map