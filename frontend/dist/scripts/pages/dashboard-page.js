...
export class DashboardPage {
    ...
    async render() {
        if (!this.dashboardSection || this.isLoading)
            return;
        this.isLoading = true;
        try {
            // Show loading state
    ...
    async renderLocalView() {
        try {
            ...
            let companies = this.getCached("companies");
            if (!companies) {
            ...
            if (this.selectedAnalysisId) {
                ...
                try {
                    ...
                    let promptsSummaryData = this.getCached(cacheKey);
            ...
            `;
                ...
                <div style="margin-top: 32px; padding: 28px; background: linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 100%); border-radius: 16px; backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 8px 32px rgba(0,0,0,0.1);">
                  ...
                  <div style="display: flex; flex-direction: column; gap: 16px;">
                    ${bestPrompts.slice(0, 5).map((p, idx) => `
          ...
          `;
                ...
                let analyses = this.getCached(cacheKey);
                if (!analyses) {
    ...
    async renderGlobalView() {
        try {
            if (this.selectedCategory) {
                ...
                let prompts = this.getCached(cacheKey);
                if (!prompts) {
        ...
        `;
                ...
                // Show category list
                let categories = this.getCached("global-categories");
                if (!categories) {
